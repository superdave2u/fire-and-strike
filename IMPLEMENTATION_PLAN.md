...
- [ ] **T1 — Money + FireGoal value objects** (`src/domain/model/Money.ts`, `src/domain/model/FireGoal.ts`)
...
- [ ] **T2 — AllocationMix value object** (`src/domain/model/AllocationMix.ts`)
  Done when: weight 0.8 → mean 0.061, std 0.156; 1.0 → 0.07/0.18; 0.0 → 0.025/0.06; rejects weights outside [0,1].
- [ ] **T3 — Percentile port + implementation** (`src/domain/ports/PercentileFn.ts`, `src/infrastructure/stats/percentile.ts`)
...
- [ ] **T4 — PercentileAggregator** (`src/domain/services/PercentileAggregator.ts`)
...
- [ ] **T5 — RandomGenerator/ReturnModel ports + GaussianReturnModel** (`src/domain/ports/RandomGenerator.ts`, `src/domain/ports/ReturnModel.ts`, `src/domain/services/GaussianReturnModel.ts`)
...
- [ ] **T6 — MonteCarloFireProjector** (`src/domain/services/MonteCarloFireProjector.ts`) + **arch-unit boundary test** (`src/domain/boundaries.test.ts` scanning `src/domain/**` for forbidden imports: react, react-dom, recharts, application, infrastructure, presentation)
...
- [ ] **T7 — StrikePaceSolver** (`src/domain/services/StrikePaceSolver.ts`)
...
- [ ] **T8 — Mulberry32Normal seeded RNG adapter** (`src/infrastructure/rng/Mulberry32Normal.ts`)
...
- [ ] **T9 — ProjectFireTrajectory use case** (`src/application/ProjectFireTrajectory.ts`, `src/application/dto.ts`)
...
- [ ] **T10 — SolveStrikePlan use case** (`src/application/SolveStrikePlan.ts`)
...
- [ ] **T11 — Chart data mappers** (`src/presentation/mappers/chartData.ts`)
...
- [ ] **T12 — InputPanel component** (`src/presentation/components/InputPanel.tsx`)
...
- [ ] **T13 — FireChart component** (`src/presentation/components/FireChart.tsx`)
...
- [ ] **T14 — StrikeChart component** (`src/presentation/components/StrikeChart.tsx`)
...
- [ ] **T15 — useFirePlan hook + App wiring** (`src/presentation/hooks/useFirePlan.ts`, `src/App.tsx`, `src/index.css`)
